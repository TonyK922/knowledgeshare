...
![图片](Linux内存分配总结.assets/640-1670489005778.jpg)
...
![图片](Linux内存分配总结.assets/640-1670489085499.png)

方式二通过 `mmap()` 系统调用中「**私有匿名映射**」的方式，在文件映射区分配一块内存，也就是从文件映射区“偷”了一块内存。如下图：

![图片](Linux内存分配总结.assets/640-1670489159099.png)
...
执行代码（**先提前说明，这里使用的 glibc 库的版本是 2.17**）：

![图片](Linux内存分配总结.assets/640-1670489273998.png)
...
![图片](Linux内存分配总结.assets/640-1670489301968.png)

从下图可以看到，通过 free 释放内存后，堆内存还是存在的，并没有归还给操作系统。

![图片](Linux内存分配总结.assets/640-1670489362782.png)
...
![图片](Linux内存分配总结.assets/640-1670489403568.png)

查看进程的内存的分布情况，可以发现最右边没有 [head] 标志，说明是通过 mmap 以匿名映射的方式从文件映射区分配的匿名内存。

![图片](Linux内存分配总结.assets/640-1670489411262.png)

然后我们释放掉这个内存看看：

![图片](Linux内存分配总结.assets/640-1670489430190.png)

再次查看该 128 KB 内存的起始地址，可以发现已经不存在了，说明归还给了操作系统。

![图片](Linux内存分配总结.assets/640-1670489447607.png)
...
![图片](Linux内存分配总结.assets/640-1670489526389.png)
...
![图片](Linux内存分配总结.assets/640-1670489547873.png)
